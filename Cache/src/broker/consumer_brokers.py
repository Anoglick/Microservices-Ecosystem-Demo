...
class ConsumerRouter:
    def __init__(self, module):
        self.module = module
        self.router = Consumer(self)
    
    async def consumer_process(self, rmq_url: str, queue_name: str):
        await self.router.connection(rmq_url=rmq_url, consumers_queue_name=queue_name)

    async def consumer_callback(self, request):
        await self.module.consumer_callback(request)
                ...
                async for message in queue_iter:
                    ...
                    try: 
                        async with message.process():
                            log.info("The Cache-service has been received a message", RMQ=rmq_url, queue=consumers_queue_name)
